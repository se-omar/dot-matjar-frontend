...
export default {
    ...
    mutations: {
        ...
        getSupplierNewData(state, supplier) {
            console.log(supplier)
            localStorage.removeItem("clickedSupplier")
    ...
    actions: {
        ...
        async updateSupplierTotalRevenue(context, { user_id, totalRevenue }) {
            console.log(user_id, totalRevenue)
            await axios.put(context.rootState.nodeHost + '/api/updateSupplierTotalRevenue', { user_id, totalRevenue })
                .then(message => {
                    alert(message.data.message)
                })
        },
        async updateSupplierAmountRecieved(context, { user_id, amountRecieved }) {
            console.log(user_id, amountRecieved)
            await axios.put(context.rootState.nodeHost + '/api/updateSupplierAmountRecieved', { user_id, amountRecieved })
                .then(message => {
                    alert(message.data.message)
                })
        },
        // async updateSupplierAmountLeft(context, { user_id, amountLeft }) {
        //     console.log(user_id, amountLeft)
        //     await axios.put(context.rootState.nodeHost + '/api/updateSupplierAmountLeft')
        //         .then(message => {
        //             alert(message.data.message)
        //         })
        // },
        async getSupplierNewData(context, user_id) {
            console.log('user id is', user_id)
            axios.put(context.rootState.nodeHost + '/api/getSupplierNewData', user_id)
                .then(supplier => {
                    context.commit('getSupplierNewData', supplier.data.data)
                })
        }
    }
}